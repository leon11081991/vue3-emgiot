...
const { groupsDDLList, goodsList, fetchGroupsDDLList, fetchGoodsList } = useFetchDropdown()
...
const goodsName = ref(goodsDefaultName)
const picker = ref({
  start: false,
  end: false,
  range: false
...
watchEffect(() => {
  ...
  if (
    props.removeSelected['goodsName'] &&
    props.removeSelected['goodsName'] !== resetGoodsNameCount
  ) {
    goodsName.value = goodsDefaultName
    resetGoodsNameCount++
  }
